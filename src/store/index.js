    ...
    async agregarGato({ commit }, payload) {
      ...
      try {
        ...
        if (!finder) {
          ...
          // const refImg = reqImg.child("/imagenes/" + payload.telefono); 
          // refImg.put(payload.imagen);
          
          storage.ref().child(`imagenes/${payload.telefono}`).getDownloadURL().then((url) => {
            nuevoGatito.img = url
            // console.log(nuevoGatito);
          });
          setTimeout(() => {
            //Aquí tengo que traer la referencia del storage y mandarla al DB
            
            commit("agregarGatoState", nuevoGatito);
           db.collection("gatitosDB").add(nuevoGatito);
          },5000);
          console.log(nuevoGatito);
        }
      } catch (error) {
